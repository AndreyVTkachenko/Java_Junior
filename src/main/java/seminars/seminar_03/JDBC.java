...
public class JDBC {
    ...
    private static List<String> selectNamesByAge(Connection connection, String age) throws SQLException {
        ...
        try (Statement statement = connection.createStatement()) {
            ...
            // where age = 1 or 1=1 это и есть SQL-инъекция = параметр(ы) передан(ы) частью запроса
        }
        */
        try (PreparedStatement statement = connection.prepareStatement("select name from person where age = ?")) { // и вот так этого можно избежать для значений не String
